/* ------------------------------------------------------------------
   One-off campaign email introducing Krystof's guided sauna sessions.
   Not wired to any send pipeline — copy the HTML into Resend/Buttondown
   (or paste `html` into resend's dashboard) when ready to send.
   Swap KRYSTOF_BOOKING_EMAIL for his real Boatyard address before sending.
   ------------------------------------------------------------------ */

export const KRYSTOF_BOOKING_EMAIL = 'krystof@theboatyardsauna.io'; // TODO: replace with Krystof's real Boatyard address

export function buildKrystofEmail(): { subject: string; html: string } {
  const subject = "Guided sauna sessions with Krystof — book him privately";
...
<html>
  <body style="margin:0;background:#0b1d2e;font-family:Helvetica,Arial,sans-serif;">
    <div style="max-width:560px;margin:0 auto;padding:40px 24px;">
      ...
      <h1 style="margin:0 0 24px;font-size:26px;color:#edf2f0;font-weight:600;">Meet Krystof, your guided session host.</h1>
      ...
      <div style="background:#edf2f0;border-radius:12px;padding:28px 26px;color:#0b1d2e;">
        ...
        <p style="margin:0 0 16px;font-size:15px;line-height:1.7;color:#33403f;">
          Krystof runs our guided sauna sessions at Wicklow Harbour — heat, cold plunge, and breathwork,
        ...
        <p style="margin:0 0 22px;font-size:15px;line-height:1.7;color:#33403f;">
          Group sessions run on the regular schedule — but if you want Krystof for a private session,
        ...
        <div style="margin:0 0 8px;padding:22px;border:2px solid #c8956c;border-radius:10px;text-align:center;background:#ffffff;">
          <p style="margin:0 0 10px;font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#a06b42;font-weight:700;">Book Krystof privately</p>
          <a href="mailto:${KRYSTOF_BOOKING_EMAIL}" style="display:inline-block;padding:14px 28px;border-radius:999px;background:#c8956c;color:#0b1d2e;font-size:15px;font-weight:700;text-decoration:none;">${KRYSTOF_BOOKING_EMAIL}</a>
          <p style="margin:12px 0 0;font-size:13px;color:#5b6b73;">Email Krystof directly to arrange a private guided session.</p>
        </div>
      </div>
